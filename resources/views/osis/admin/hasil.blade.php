...
@section('content')
<div class="space-y-6" x-data="osisHasil('{{ route('osis.hasil.data', $pemilihan) }}')">
    <div>
...
function osisHasil(url) {
    ...
    return {
        ...
        async muat() {
            ...
            if (chartSiswa) { chartSiswa.data = cfg(d.siswa, '#7ba088').data; chartSiswa.update(); }
            else chartSiswa = new Chart(document.getElementById('chartSiswa'), cfg(d.siswa, '#7ba088'));
            if (chartGuru) { chartGuru.data = cfg(d.guru, '#3b82f6').data; chartGuru.update(); }
            else chartGuru = new Chart(document.getElementById('chartGuru'), cfg(d.guru, '#3b82f6'));
        },
    };
}
</script>
