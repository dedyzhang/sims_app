...
<div class="max-w-3xl mx-auto space-y-5">
    ...
    <div class="space-y-3">
        ...
        <div class="card p-5" x-data="soalForm({
                ...
                open: false,
                rendered: false,
                fetchUrl: '{{ route('bank-soal.soal.data', [$pelajaran, $s]) }}'
              })"
             x-init="_rootEl = $el">
            ...
            <form @submit="encodeForm" x-show="open" x-cloak x-transition method="POST" action="{{ route('bank-soal.soal.update', [$pelajaran, $s]) }}" class="mt-4 space-y-3 border-t border-slate-100 dark:border-slate-700 pt-4">
    ...
    <div class="card p-5" x-data="soalForm({ tipe: 'mcq', teks_soal: '', poin: 1, poin_salah: '', penjelasan: '', opsi: [{teks:'',benar:true},{teks:'',benar:false}], pasangan: [{kiri:'',kanan:''},{kiri:'',kanan:''}], kunci_esai: '', skor_mode: 'all_or_nothing', open: true })"
        ...
        <form @submit="encodeForm" method="POST" action="{{ route('bank-soal.soal.store', $pelajaran) }}" class="space-y-3">
            @csrf
            @include('ujian.partials.soal-fields')
