        ...
        <a href="{{ route('classroom.assignment.show', [$a, 'class' => $classroom->uuid]) }}" class="card p-4 flex items-center gap-3 hover:border-primary transition">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if($a->status==='draft')<span class="text-[11px] px-2 py-0.5 rounded bg-amber-100 text-amber-700">Draf</span>@if($canManage)<span role="button" @click.prevent.stop="fetch('{{ route('classroom.assignment.publish', $a) }}', {method: 'POST', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>window.location.reload())" class="text-[11px] px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition">Terbitkan</span>@endif @endif
