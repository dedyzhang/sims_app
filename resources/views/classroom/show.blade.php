    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-4 flex items-center justify-between flex-wrap gap-3">
            <div class="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-3 flex-wrap">
                ...
                @if($classroom->semester)<span> &bull; {{ $classroom->semester->semester }} {{ $classroom->semester->tahun }}</span>@endif
                <span> &bull; @foreach($classroom->kelas as $k)<span class="font-medium">{{ $k->tingkat }}{{ $k->kelas }}</span>@if(!$loop->last), @endif @endforeach</span>
                <span> &bull; {{ $classroom->members->count() }} siswa</span>
            </div>
            @if($classroom->rombel)
    ...
    <div x-show="tab==='tugas'" x-cloak class="space-y-4">
        ...
        <a href="{{ route('classroom.assignment.show', [$a, 'class' => $classroom->uuid]) }}" class="card p-4 flex items-center gap-3 hover:border-primary transition">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-slate-400 mt-0.5">Nilai maks {{ $a->max_score }}@if($a->due_at) &bull; Batas {{ $a->due_at->locale('id')->translatedFormat('d M Y H:i') }}@endif
                    @if($me->access==='siswa')
                        @php $sub=$mySubmissions[$a->uuid]??null; @endphp
                        &bull;
                        @if($sub && $sub->status==='graded')
                            <span class="text-emerald-600 font-semibold">@if($a->hide_scores)Tugas sudah dikoreksi@elseNilai {{ $sub->score }}@endif</span>
                        @elseif($sub)
                            <span class="text-sky-600">Sudah dikumpulkan</span>
                        @else
                            <span class="text-amber-600">Belum dikumpulkan</span>
                        @endif
                    @endif
                </p>
            </div>
