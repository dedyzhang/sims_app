...
<div class="space-y-4">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div class="flex items-center gap-3">
            ...
            @endif
            <div>
                <h1 class="page-title">Jadwal per Guru</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Tampilan mingguan satu guru</p>
            </div>
        </div>
        <form method="GET" action="{{ route('jadwal.guru') }}">
            <select name="guru" onchange="this.form.submit()" class="form-select py-2.5 text-sm w-auto">
                        ...
                        <tr>
                            ...
                            <td class="p-1 align-middle text-center">
                                ...
                                @php
                                    ...
                                    $kelasNama = $j->kelas ? $j->kelas->tingkat.$j->kelas->kelas : '';
                                @endphp
                                <div class="kcell rounded-lg px-1 py-2.5 flex flex-col items-center justify-center" style="background:color-mix(in srgb,var(--cp) 11%,#fff)"
                                     data-pnama="{{ $pnama }}" data-gnama="Kelas {{ $kelasNama }}" title="{{ $pnama }} — Kelas {{ $kelasNama }}">
                                    <span class="font-bold text-[12.5px] leading-none text-slate-700 dark:text-slate-200">{{ $short }}</span>
                                    <span class="text-[10px] text-slate-500 mt-0.5 font-semibold leading-none">{{ $kelasNama }}</span>
                                </div>
                                @else
...
(function(){
    ...
    if(!tip) return;
    let active = null;
    const personSvg = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M2 20V6a2 2 0 012-2h16a2 2 0 012 2v14"/><path d="M2 20h20"/><path d="M12 20v-8"/><path d="M8 12h8"/><path d="M8 16h8"/></svg>';
    function show(c){
        const pn = c.dataset.pnama, gn = c.dataset.gnama;
        if(!pn) return;
        tip.innerHTML = `<div class="jt-pel">${pn}</div><div class="jt-guru">${personSvg}${gn || 'Kelas tidak diketahui'}</div>`;
