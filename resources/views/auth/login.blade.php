...
<body class="min-h-screen bg-slate-50 flex overflow-x-hidden relative">
    ...
    <div class="login-panel w-full lg:w-1/2 min-h-screen flex flex-col items-center justify-center p-6 relative overflow-y-auto bg-slate-50">
        ...
        <div x-data="loginApp()" class="login-card-shell w-full max-w-[400px] z-10 rise-up">
            ...
            {{-- Unduh Aplikasi: iPhone PWA selalu tampil. APK/Windows hanya kalau admin
                 aktifkan & file-nya ada. Route tamu, tanpa login. --}}
            @php
            ...
            <div class="mt-6 pt-5 border-t border-slate-200/70">
                ...
                <div class="flex items-center justify-center gap-3 flex-wrap">
                    ...
                    @endif
                    <a href="{{ route('guest.app.ios') }}" class="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl border border-slate-200 bg-white hover:bg-slate-50 active:scale-[0.98] transition shadow-sm">
                        <span class="grid place-items-center w-8 h-8 rounded-xl bg-slate-100 flex-shrink-0">
                            <svg viewBox="0 0 24 24" class="w-[17px] h-[17px]" aria-hidden="true">
                                <path fill="#111827" d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701z"/>
                            </svg>
                        </span>
                        <span class="text-left leading-tight">
                            <span class="block text-[10px] text-slate-400 font-semibold">iPhone / iPad</span>
                            <span class="block text-xs font-bold text-slate-700">Pasang App</span>
                        </span>
                    </a>
                    @if($loginWinOk)
